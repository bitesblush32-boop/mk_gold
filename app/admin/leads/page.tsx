...
export default function LeadsPage() {
  ...
  async function saveRemarks(lead: Lead) {
    ...
    try {
      ...
      setLeads((prev) =>
        prev.map((l) =>
          l.id === lead.id ? { ...l, notes: remarks[lead.id] ?? "" } : l,
        ),
      );
      setFiltered((prev) =>
        prev.map((l) =>
  ...
  function applyFilters() {
    ...
    if (dateFrom) {
      result = result.filter(
        (l) => new Date(l.created_at as string) >= new Date(dateFrom),
      );
    }
    if (dateTo) {
    ...
    try {
      const res = await fetch("/api/leads", {
        ...
        body: JSON.stringify({
          ...addForm,
          source: "admin",
          purity_karat: addForm.purity_karat
            ? Number(addForm.purity_karat)
            : undefined,
  ...
  return (
    <div className="mk-admin-page">
      ...
      <p className="mk-admin-subtitle">
        All website leads. {leads.length} total.
      </p>

      {/* Add Lead Form */}
      {showAdd && (
        <div
          className="mk-admin-section"
          style={{
            ...
            >
              ...
              <div className="mk-admin-field">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAddForm((f) => ({ ...f, gold_type: e.target.value }))
                  }
                >
              ...
              <div className="mk-admin-field">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAddForm((f) => ({ ...f, purity_karat: e.target.value }))
                  }
                >
              ...
              <div className="mk-admin-field">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setAddForm((f) => ({ ...f, weight_grams: e.target.value }))
                  }
      ...
      >
        {loading ? (
          <p className="mk-admin-muted" style={{ padding: "var(--s-6)" }}>
            Loading…
          </p>
        ) : filtered.length === 0 ? (
          <p className="mk-admin-muted" style={{ padding: "var(--s-6)" }}>
                ...
                {filtered.map((lead) => (
                  <React.Fragment key={lead.id}>
                    <tr
                      ...
                      onClick={() =>
                        setExpandId(expandId === lead.id ? null : lead.id)
                      }
                    ...
                    >
                      ...
                      <td>
                        {lead.purity_karat ? `${lead.purity_karat}K` : "—"}
                      </td>
                      <td>
                        {lead.weight_grams ? `${lead.weight_grams}g` : "—"}
                      </td>
                      <td>
                        {(() => {
                      ...
                      >
                        <button
                          ...
                          style={{
                            ...
                            background:
                              deletingId === lead.id ? "#e8b4b8" : "#fef2f2",
                            color: "#c0392b",
                      ...
                      <tr className="mk-admin-expand-row">
                        <td colSpan={9}>
                          <div className="mk-admin-expand-grid">
                            <div>
                              <span className="mk-admin-expand-label">
                                Email
                              </span>
                              {cell(lead.email)}
                            </div>
                            ...
                            <div>
                              ...
                              {cell(lead.utm_source)}
                            </div>
                            <div>
                              <span className="mk-admin-expand-label">
                                UTM Medium
                              </span>
                              {cell(lead.utm_medium)}
                            </div>
                            <div>
                              <span className="mk-admin-expand-label">
                                UTM Campaign
