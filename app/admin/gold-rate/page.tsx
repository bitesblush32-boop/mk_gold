...
export default function GoldRatePage() {
  ...
  async function handleSet(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        setMessage({
          type: "ok",
          text: "Manual rate override set successfully.",
        });
        await fetchData();
      } else {
  ...
  async function handleClear() {
    ...
    try {
      ...
      if (res.ok) {
        setMessage({
          type: "ok",
          text: "Override cleared. Showing live MCX rate.",
        });
        setOverride(null);
        if (liveRate) {
          setRates({
  ...
  const isOverrideActive = Boolean(
    override?.is_manual &&
    (!override.override_until ||
      new Date(override.override_until) > new Date()),
  );
  ...
  return (
    <div className="mk-admin-page">
      ...
      <p className="mk-admin-subtitle">
        When set, the homepage and all pages will show this rate instead of the
        live MCX rate.
      </p>
      ...
      {isOverrideActive && (
        <div className="mk-admin-warning-banner">
          Manual rate override is active. Users are seeing your set rate, not
          the live MCX rate. Override expires at{" "}
          {fmtDateTime(override!.override_until)}. Click &ldquo;Clear
          Override&rdquo; to restore the live rate.
        </div>
      )}
          ...
          >
            <div>
              <div className="mk-admin-rate-row">
                ...
                <span className="mk-admin-rate-val">
                  ₹{Number(override!.rate_24k).toLocaleString("en-IN")}/g
                </span>
                <span
                  className="mk-admin-rate-karat"
                  style={{ marginLeft: "1.5rem" }}
                >
                  22K
                </span>
                <span className="mk-admin-rate-val">
                  ₹{Number(override!.rate_22k).toLocaleString("en-IN")}/g
                </span>
              </div>
      ...
      <div className="mk-admin-section" style={{ marginTop: "var(--s-5)" }}>
        ...
        <form onSubmit={handleSet} noValidate>
          ...
          <div
            className="mk-admin-field"
            style={{ maxWidth: "220px", marginTop: "var(--s-4)" }}
          >
