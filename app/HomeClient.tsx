    ...
    >
      ...
      <div>
        ...
        <select
          ...
          value={form.city}
          onChange={(e) =>
            setForm((f) => ({ ...f, city: e.target.value, pincode: "" }))
          }
        >
...
function loadMapsSDK(): Promise<void> {
  ...
  if (win.google?.maps?.importLibrary) {
    _mapsReady = Promise.resolve();
    return _mapsReady;
  }

  // loading=async: libraries loaded on demand via importLibrary after bootstrap
...
const CITY_CENTERS: Record<City, { lat: number; lng: number; zoom: number }> = {
  Bangalore: { lat: 12.94, lng: 77.61, zoom: 12 },
  Mysore: { lat: 12.31, lng: 76.64, zoom: 13 },
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function makePinElement(PinElement: any, isActive: boolean): any {
...
function GoogleCityMap({
  ...
  useEffect(() => {
    ...
    // Clear old markers immediately (sync) before async init
    ...
    (async () => {
      try {
        ...
        const { Map } = (await gm.importLibrary("maps")) as any;
        const { AdvancedMarkerElement, PinElement } = (await gm.importLibrary(
          "marker",
        )) as any;
        ...
        markersRef.current = cityBranches.map((branch) => {
          ...
          const pin = makePinElement(PinElement, isActive);
  ...
  return (
    <div className="sc-city-panel reveal delay-3">
      <div className="sc-city-panel__inner">
        {/* LEFT: Google Map */}
        <div
          className="sc-gmap-container"
          ref={mapDivRef}
          aria-label={`${city} branch map`}
        >
          {mapError && (
            <div
              style={{
          ...
          {activeBranch ? (
            <div className="sc-branch-detail__card" key={activeBranch.slug}>
              <div className="sc-branch-detail__header">
                <span className="sc-branch-detail__name">
                  {activeBranch.name}
                </span>
                <button
                  className="sc-branch-detail__close"
              ...
              <p className="sc-branch-detail__hours">
                {activeBranch.openHours.days}&nbsp;·&nbsp;
                {activeBranch.openHours.time}
              </p>
              <div className="sc-branch-actions">
                ...
                <a href={`/${activeBranch.slug}`} className="sc-branch-action">
                  View Branch
                </a>
              </div>
            </div>
          ...
          {CITIES.map((city) => {
            ...
            return (
              ...
              >
                ...
                {isActive ? (
                  <p className="sc-city-count">
                    {count} {count === 1 ? "branch" : "branches"}
                  </p>
                ) : (
                  <p
                    className="sc-city-count"
                    style={{ color: "var(--mist)", fontStyle: "italic" }}
                  >
                    Coming Soon
                  </p>
                )}
...
const TRUST_BADGES = [
  "GST Registered",
  "ISO 9001:2015",
  "XRF Certified" /* , '16 Physical Branches' */,
] as const; // was: 16 Physical Branches
        ...
        <div className="mk-trust__left">
          ...
          <div className="reveal delay-1">
            ...
            <div className="sc-trust-stats reveal delay-2">
              {/* 4.9 Google Rating */}
              <div
                className="sc-trust-stat sc-trust-stat--rating"
                style={{
              ...
              >
                ...
                <div
                  className="sc-trust-stat__stars"
                  aria-label="4.9 out of 5 stars"
                >
                  {[0, 1, 2, 3, 4].map((i) => (
                    <span
                      key={i}
                      className="sc-trust-star"
                      style={{
                        animation:
                          "mk-star-in 0.35s cubic-bezier(0.34,1.56,0.64,1) both",
          ...
          <div
            className="mk-trust__badges reveal delay-3"
...
function LocalStepsSection() {
  return (
    <section
      ...
      style={{
        backgroundColor: "#fff",
        paddingTop: "calc(var(--section) - 25px)",
      }}
      id="how-it-works"
    >
      <div className="mk-container">
    ...
    <>
      ...
      >
        ...
        >
          ...
          <a
            href={`tel:${process.env.NEXT_PUBLIC_PHONE_DEFAULT ?? "+917019500600"}`}
            className="sc-bn-btn sc-bn-btn--call"
          >
            Call Us
          </a>
          <a
...
export default function HomePage({
  ...
  const [banners, setBanners] =
    useState<{ src: string | null; alt: string; src_mobile?: string | null }[]>(
      initialBanners,
    );
  ...
  useEffect(() => {
    fetch("/api/reviews")
      ...
      .catch(() => {
        /* keep empty — section shows loading state */
      });
  }, []);
            ...
            {!rateUnlocked && (
              <div className="sc-rate-top-grid">
                ...
                <div className="sc-gate-copy reveal delay-1">
                  ...
                  >
                    Get an instant, accurate estimate of your gold&apos;s worth
                    based on current live market rates and purity. No obligation
                    — just clarity.
                  </p>
                ...
                <div className="sc-gate-card reveal delay-2">
                  <div>
                    ...
                    <p className="sc-gate-subtitle">
                      Enter your details to unlock the live calculator
                    </p>
                  </div>
                  <CallbackForm onSuccess={() => setRateUnlocked(true)} />
                  <p className="sc-gate-privacy">
          ...
          <div style={{ overflow: "hidden", paddingBottom: "0.5rem" }}>
            <div className="sc-reviews-track">
              {[...googleReviews, ...googleReviews].map((t, i) => (
                <div
                  key={i}
                  className="sc-review-card sc-review-card--carousel"
                >
                  <span className="sc-google-badge">Google</span>
                  <div
                    className="sc-review-stars"
                  ...
                  <div className="sc-review-author">
                    <div className="sc-review-avatar" aria-hidden="true">
                      {t.initials}
                    </div>
                    <div>
