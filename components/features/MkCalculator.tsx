...
export function MkCalculator({
  ...
  async function handleGateSubmit(e: React.FormEvent) {
    ...
    const estimatedValue =
      estRate && weightGrams
        ? Math.round(estRate * weightGrams * 0.975)
        : undefined;
  ...
  const mid = weightNum > 0 ? Math.round(baseRate * weightNum * 0.975) : null;
  ...
  if (!unlocked) {
    return (
      <div className={`mk-calc mk-calc--${variant}`}>
        ...
        <p
          className="mk-calc__gate-sub"
          style={{
            fontFamily: "Poppins,sans-serif",
            fontSize: "var(--t-sm)",
            color: isDark ? "rgba(255,255,255,0.65)" : "var(--ink-mid)",
        ...
        >
          ...
          <select
            ...
            value={gateForm.city}
            onChange={(e) =>
              setGateForm((f) => ({ ...f, city: e.target.value, pincode: "" }))
            }
          >
