...
export function getChannel(lead: {
  ...
  if (
    ...
    if (src.includes("google"))
      return { label: "Google Ads", color: "#1a73e8" };
    if (src.includes("facebook") || src.includes("fb"))
      return { label: "Meta Ads", color: "#1877f2" };
    if (src.includes("instagram"))
      return { label: "Instagram Ads", color: "#c13584" };
    return { label: "Paid Ad", color: "#7B2C91" };
  }

  // Organic search
  if (med === "organic" || (src === "google" && !med))
    return { label: "Google Organic", color: "#188038" };
  ...
  if (raw.includes("calculator"))
    return { label: "Calculator", color: "#C9A940" };
