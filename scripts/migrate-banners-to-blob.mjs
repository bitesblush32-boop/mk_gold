...
async function run() {
  for (const { id, file } of BANNERS) {
    ...
    console.log(
      `[${id}] Uploading to Vercel Blob (${(buffer.length / 1024).toFixed(0)} KB)…`,
    );
  ...
  const rows =
    await sql`SELECT id, src, alt, "order" FROM hero_banners ORDER BY "order"`;
  console.log("Current hero_banners:");
  for (const r of rows) {
    console.log(
      `  [${r.id}] order=${r.order} alt="${r.alt}"\n       src=${r.src}`,
    );
  }
